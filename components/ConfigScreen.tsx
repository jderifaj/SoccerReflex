...
const ConfigScreen: React.FC<ConfigScreenProps> = ({
  ...
  const radius = 16;
  const perimeter =
    2 * (btnSize.width + btnSize.height - 4 * radius) + 2 * Math.PI * radius;
  ...
  const handleLaunch = () => {
    // Note: AudioContext initialization must be triggered by user interaction
    onStart();
  };
  ...
  return (
    <div className="max-w-md mx-auto p-6 pb-20">
      ...
      <section className="mb-10">
        <h2 className="text-sm font-bold mb-4 flex justify-between items-center text-slate-400 uppercase tracking-widest">
          ...
          <span className="text-xs font-mono">
            {config.selectedColors.length}/5
          </span>
        </h2>
      ...
      <ChangeInterval
        ...
        onChange={(val) =>
          setConfig((prev) => ({ ...prev, delaySeconds: val }))
        }
      />
      ...
      <VoiceToggle
        ...
        onToggle={() =>
          setConfig((prev) => ({ ...prev, voiceEnabled: !prev.voiceEnabled }))
        }
      />
      ...
      <section className="mb-16">
        ...
        >
                ...
                {config.selectedColors.map((colorName, i) => {
                  ...
                  return (
                    <rect
                      ...
                      style={{
                        filter: `drop-shadow(0 0 12px ${colorDef?.hex}aa)`,
                      }}
                    />
                  );
                })}
          ...
          <Button
            variant="primary"
            onClick={() => {}} // onClick handled by parent div
      ...
      {favorites.length > 0 && (
        <section className="mb-10">
          <h2 className="text-sm font-bold mb-4 text-slate-400 uppercase tracking-widest border-t border-white/5 pt-8">
            My Presets
          </h2>
          <div className="space-y-3">
            {favorites.map((fav) => (
              <div
                key={fav.id}
                className="flex items-center gap-3 bg-slate-900/40 p-4 rounded-2xl border border-white/5 shadow-lg group hover:bg-slate-900 transition-colors"
              >
                <button
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    {fav.selectedColors.map((c) => (
                      <div
                        key={c}
                        className={`w-2.5 h-2.5 rounded-full ${COLORS.find((col) => col.name === c)?.bgClass}`}
                      />
                    ))}
                    <span className="text-[10px] text-slate-500 font-mono ml-2 uppercase">
                      {fav.delaySeconds}S interval
                    </span>
                  </div>
                </button>
                <button
